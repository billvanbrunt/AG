...
public partial class controls_ctrlDefaultAddStudent : System.Web.UI.UserControl
{
    ...
    protected void OpenStudentInformation(string vStudentId)
    {
        ...
        try
        {
            if (ds.Tables[1].Rows[0]["ECStatusCodes"].ToString() != "")
            {
                txtECStatus.Text = ds.Tables[1].Rows[0]["ECStatusCodes"].ToString();
            }
            else
            {
                txtECStatus.Text = "";
        ...
        {
            string ll = e.Message.ToString();
        }
    }
    protected void EligibilityInfo_Click(object sender, EventArgs e)
    {
        Response.Redirect("/DefaultAddEligibilityInfo.aspx?id=" + lStudentId);
    }
    ...
    {
        Response.Redirect("/DefaultAddTestingInformation.aspx?id=" + lStudentId);
    }
}